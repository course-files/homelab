...
while true; do
    ...
    echo "2) CPU Usage"
    echo "3) Memory Usage"
    read -p "Your choice: " SORT_CHOICE || { echo "Input interrupted. Exiting."; log_event "INPUT_INTERRUPTED" "$USERNAME"; exit 1; }

    # Step 9: Ask how many processes to display.
    echo ""
    read -p "Enter the number of processes to display (press Enter for all): " NUM_PROCESSES \
        || { echo "Input interrupted. Exiting."; log_event "INPUT_INTERRUPTED" "$USERNAME"; exit 1; }

    # Validate: must be a positive integer, or empty (meaning "show all").
    if [ -n "$NUM_PROCESSES" ]; then
        if ! [[ "$NUM_PROCESSES" =~ ^[1-9][0-9]*$ ]]; then
            echo "Invalid input: '$NUM_PROCESSES' is not a positive integer. Displaying all processes."
            NUM_PROCESSES=""
        fi
    fi

    # Step 10: List the processes for the specified user in a paginated format.
    if ! [ "$CONTINUE" = "q" ]; then
        log_event "PROCESSES_VIEWED" "$USERNAME"

        # Build the limiting filter conditionally.
        # If NUM_PROCESSES is set, pipe through head; otherwise pass all output through.
        HEAD_CMD="cat"
        if [ -n "$NUM_PROCESSES" ]; then
            HEAD_CMD="head -n $NUM_PROCESSES"
        fi
        ...
        case "$SORT_CHOICE" in
            1) ps -u "$USERNAME" -f | sort -k 2 -n  | $HEAD_CMD | less || true ;;
            2) ps -u "$USERNAME" -f | sort -k 3 -rn | $HEAD_CMD | less || true ;;
            3) ps -u "$USERNAME" -f | sort -k 4 -rn | $HEAD_CMD | less || true ;;
            *) echo "Invalid choice. Displaying unsorted list."
               ps -u "$USERNAME" -f | $HEAD_CMD | less || true ;;
        esac
    else
