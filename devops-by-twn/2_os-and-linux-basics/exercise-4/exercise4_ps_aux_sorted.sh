...
while true; do
    ...
    if ! [ "$CONTINUE" = "q" ]; then
        ...
        case "$SORT_CHOICE" in
            1) ps -u "$USERNAME" -f | sort -k 2 -n | less || true ;;
            2) ps -u "$USERNAME" -f | sort -k 3 -rn | less || true ;;
            3) ps -u "$USERNAME" -f | sort -k 4 -rn | less || true ;;
            *) echo "Invalid choice. Displaying unsorted list."; ps -u "$USERNAME" -f | less || true ;;
        esac
    else
